...
public class example32_02_1_M { //РЕАЛИЗАЦИЯ С МЕТОДАМИ
	...
	public static int [] EnterArr(int [] array) throws InputMismatchException {//метод заполнения массива элементами
		Scanner in=new Scanner(System.in); //инициализация ввода
		System.out.println("Введите массив целых чисел:");
		for (int i=0; i<array.length; ++i) {array[i]=in.nextInt();}
		return array;
	}
	public static void Midlarr(int [] array) throws ArithmeticException {
	...
	try {
				...
				int[] array=new int[EnterSize()]; //создание массива + метод ввода длины
				array=EnterArr(array); //метод заполнения
				Midlarr(array); //метод вычисления среднего значения  
